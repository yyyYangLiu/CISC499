...
def execute(num, program, prog_length, register_dic):
    if_statement = True
    for i in range(0, prog_length):
        if if_statement:
            if program[i][1] == "+":
...
def main():
    ...
    num = 0
    while num < totalLabel:
        program = dic_program["L" + str(num)]
        print("This is " + "L" + str(num) + "    " + str(program))
        num, register_dic = execute(num, program, len(program), register_dic)
    print(register_dic)
